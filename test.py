...
def test_reader_and_extractor(file_path):
    ...
    for read_file_path in read_file_paths:
        ...
        feedback, lengths, results = extractor.extract()
        print(feedback)
        print(f"Metadata lengths by method: {lengths}")
        # Print a subset of the metadata
        for method, metadata in results.items():
            subset_metadata = {k: metadata[k] for k in list(metadata)[:2]} if not metadata.get("error") else metadata
            print(f"Extracted Metadata (subset) using {method} for {read_file_path}: {subset_metadata}")
